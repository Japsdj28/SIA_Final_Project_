...
session_start();
/**
 * ALTER TABLE products ADD product_quantity INT(11) NOT NULL AFTER `product_price`;
 	UPDATE `products` SET product_quantity = 1000 WHERE 1;
...
class Products
{
	...
	public function getProducts(){
		$q = $this->con->query("SELECT p.product_id, p.product_name, p.product_price,p.product_quantity, p.product_desc, p.product_image, p.product_keywords FROM products");
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ecommerce-app-h/product_images/".$uniqueImageName)) {
					...
					$q = $this->con->query("INSERT INTO `products`(`product_cat`, `product_brand`, `product_title`, `product_quantity`, `product_price`, `product_desc`, `product_image`, `product_keywords`) VALUES ('$category_id', '$brand_id', '$product_name', '$product_quantity', '$product_price', '$product_desc', '$uniqueImageName', '$product_keywords')");
